...
(function() {
	...
	app.each = function(arr, fn) {
		for (let i = 0; i < arr.length && fn(arr[i]) !== false; i++) {}
	};
	...
	app.on = function(elem, event, fn) {
		if (elem.nodeName || elem === window)
			...
			const events = elem.__events__ = elem.__events__ || {};
	...
	app.off = function(elem, event, fn) {
		if (elem.nodeName || elem === window)
			...
			const callbacks = elem.__events__ && elem.__events__[event];
			let cb, i = -1;
			if (callbacks) {
	...
	app.localizeElements = function(document) {
		...
		app.each(document.querySelectorAll('[i18n-attr]'), function($elem) {
			const m = $elem.getAttribute('i18n-attr').split('|');
