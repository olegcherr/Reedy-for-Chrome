...
(function(app) {
	...
	app.Parser = function(raw) {
		...
		var api = this,
			sign = '~NL'+(+(new Date())+'').slice(-5)+'NL~',
			text = api.text = raw.replace(/\n|\r/gm, sign).replace(/\s+/g, ' ').replace(new RegExp(sign, 'g'), '\n'),
			textLen = text.length,
			data = [],
			wid = -1;
		...
		api.parse = function() {
			var entityAnalysis = app.get('entityAnalysis'),
				startPos, position = 0,
				startChar, char, prevChar, prev2Char, nextChar, next2Char, partAfter,
			...
			while (position < textLen) {
				...
				for (;position <= textLen; position++) {
					...
					if (REX_CHAR_BOUND.test(char)) {
						...
						if (REX_CHAR_DELAYED.test(prevChar)) {
							isDelayed = true;
						}
						
						while (REX_CHAR_BOUND.test(text[++position])) {}
