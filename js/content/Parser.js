...
(function(app) {
	...
	function splitWordIfNeeded(str) {
		const dashIndex = str.indexOf("-"), slashIndex = str.indexOf("/"), uscoreIndex = str.indexOf("_");
		if (str.length > 13 || str.length > 9 && (dashIndex > -1 || uscoreIndex > -1 || slashIndex > -1)) {
			...
			let parts = Math.ceil(str.length / 8),
	...
	function isSentenceEnd(str) {
		const m = REX_SENTENCE_END.exec(str);
	...
	const REX_SENTENCE_END = /(?:\.|…|!|\?|;)([^.…!?;]*)$/,
		...
		patterns_level2 = [
			...
			function(i, token, tokenStr, stack, stackStr) {
				...
				const host = '(' + '(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\\.)+[a-z]{2,6}'
						+ '|' + '(?:\\d{1,3}\\.){3}\\d{1,3}' + ')',
...
//			}
	...
	app.parse1 = function(raw) {
		const data = [];
		let char, prevChar, isSpace, isNewLine,
			charType, prevType, token;
