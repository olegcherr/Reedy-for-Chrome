...
(function(app) {
	...
	app.Sequencer = function(raw, data) {
		...
		function getTiming(complexity) {
			const gradualAccel = app.get('gradualAccel'),
		...
		function next(justRun) {
			...
			if (api.index >= length-1) {
				...
				function doUpdate() {
					const hyphenated = wasLaunchedSinceOpen && !justRun && app.get('hyphenation') ?
							token.toHyphenated() : [token.toString()];
					let part;
		...
		const api = this;
		let wasLaunchedSinceOpen = false,
			length = data.length,
		...
		api.getContext = function(charsLimit) {
			...
			return {
				before: raw.substring(charsLimit ?
						Math.max(token.startIndex-charsLimit, 0) : 0, token.startIndex).trim(),
				after: raw.substring(token.endIndex, charsLimit ?
						Math.min(token.endIndex+charsLimit, raw.length) : raw.length).trim()
			};
		};
		...
		api.getSequel = function() {
			...
			const fromIndex = api.index+1,
				res = [];

			for (let t, i = 0; i < 10 && (t = data[fromIndex+i]); i++) {
				res.push(t.toString());
