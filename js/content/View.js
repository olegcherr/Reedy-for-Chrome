...
(function(app) {
	...
	function createControl(modifiers, $appendTo, title) {
		const $btn = createElement('div',
			cls.apply(null, ('control_'+modifiers.join(' control_')).split(' ').concat('control')), $appendTo);
	...
	app.View = function() {
		...
		function onSettingsUpdate(e, key) {
			updateWrapper();
		...
		function onScrollerBarMousedown(e) {
			...
			const rectBarWrap = app.offset($scrollerBarWrap),
				rectBar = app.offset($scrollerBar),
				startY = e.pageY;
			let lastPos = +$scrollerBar.style.top.replace('%','') || 0;
		...
		function onScrollerBgMousedown(e) {
			...
			const rectBg = app.offset($scrollerBg);
		...
		function updateFocusPoint() {
			const rect = $focusDashes.getBoundingClientRect();
		...
		function updateTimeLeft() {
			const timeLeft = sequencer.getTimeLeft(), parts = [];
